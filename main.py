...
def main_menu():
    ...
    kb.add(btn_mylink, btn_edit, btn_sponsor, btn_childinfo, btn_stop, btn_rules, btn_complaint)
    return kb
...
async def process_callback_show_and_edit(callback_query: types.CallbackQuery):
    ...
    if current_user["description"] == "":
        ...
        await bot.send_message(callback_query.from_user.id, "Вы ещё не заполнили информацию о себе.", reply_markup=inline_kb)
    else:
        text = "Вот что вы ранее написали о себе:\n"
        text += current_user["description"]
        button_yes = types.InlineKeyboardButton("Редактировать", callback_data="edit_info")
        button_no = types.InlineKeyboardButton("Сохранить", callback_data="no_action")
...
async def process_callback_return_to_main(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id)
    await bot.send_message(callback_query.from_user.id, "Редактирование отменено.", reply_markup=main_menu())
...
async def process_callback_edit_my_info(callback_query: types.CallbackQuery):
    ...
    if "username" not in callback_query.from_user:
        ...
        await bot.send_message(callback_query.from_user.id,
                               "Установите имя пользователя в настройках Telegram, чтобы другие участники могли вам написать!",
                               reply_markup=kb)
...
async def username_check(message: types.Message, state: FSMContext):
    if "username" not in message.from_user:
        await message.answer("Пожалуйста, проверьте настройки Telegram ещё раз. Имя пользователя должно иметь вид @username")
    else:
        update_info(message.from_user.id, "username", message.from_user["username"])
        await message.answer("Спасибо!\nТеперь опишите, чем Вы можете быть интересны и полезны другим интернет бизнесменам:",
...
async def getting_my_info(message: types.Message, state: FSMContext):
    if len(message.text) < 10:
        await message.answer("Пожалуйста, напишите о себе немного подробнее...")
    else:
        update_info(message.from_user.id, "description", message.text)
        text = "Отлично!\nИнформация сохранена и будет тиражироваться среди участников, пришедших по вашей реферальной ссылке."
        await state.finish()
...
async def meet_sponsor(user_id):
    ...
    while n < depth:
        ...
        if current_user["status"] == 1 and current_user["description"] != "":
            ...
            n += 1
    if status == 3:
        button_fill = types.InlineKeyboardButton("Приступить", callback_data="edit_info")
        inline_kb = types.InlineKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True).add(button_fill)
        await bot.send_message(user_id, "Теперь вы можете заполнить информацию о себе", reply_markup=inline_kb)
